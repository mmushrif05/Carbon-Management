...
async function loadAllData() {
  ...
  // Setup real-time listeners (avoid re-rendering pages with forms)
  DB.onEntriesChange(data => { state.entries = data; if (state.page === 'dashboard' || state.page === 'entry_a13' || state.page === 'monthly' || state.page === 'cumulative') navigate(state.page); });
  ...
  DB.onSubmissionsChange(data => { state.submissions = data; if (state.page === 'approvals') navigate(state.page); });
}
