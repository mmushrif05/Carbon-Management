...
function buildSidebar() {
  ...
  if(r==='contractor'||r==='consultant'){const draftCount=r==='contractor'?DB.getDraftEntries().length:0;items.push({section:"Data Entry"},{id:'entry_a13',icon:'\ud83e\uddf1',label:'A1-A3 Materials',badge:draftCount},{id:'boq_upload',icon:'\ud83d\udcc2',label:'BOQ Upload'},{id:'entry_a5',icon:'\u26a1',label:'A5 Site Emissions'});}
  ...
  items.push({section:"Reports"},{id:'monthly',icon:'\ud83d\udcc5',label:'Monthly Report'},{id:'cumulative',icon:'\ud83d\udcc8',label:'Cumulative'},{id:'calculations',icon:'\ud83d\uddd2\ufe0f',label:'Calculations'});
...
function navigate(page) {
  ...
  const titles={dashboard:["Dashboard","Project carbon performance & sustainability metrics"],entry_a13:["A1-A3 Material Entry","Enter material quantities and emission factors (Full ICE Database)"],boq_upload:["BOQ Upload","Upload Excel or CSV Bill of Quantities \u2014 Auto-match to ICE database"],entry_a5:["A5 Site Emissions","Monthly fuel and water consumption"],approvals:["Approval Workflow","Review and approve carbon data"],monthly:["Monthly Report","Monthly emissions breakdown \u2014 Export PDF/Excel"],cumulative:["Cumulative Report","Running totals and trends \u2014 Export PDF/Excel"],calculations:["Detailed Calculations","Verify emission calculations, MEP coverage & methodology"],baselines:["Baseline EFs","Full ICE Database \u2014 Emission factor reference data"],team:["Team Management","Invite and manage project team members"],organizations:["Organizations & Assignments","Manage firms, companies, and consultant-contractor assignments"],certifications:["Certifications","Track sustainability certification credits"],integrations:["API Hub","External integrations and data sources"],tender_entry:["Tender Quantities","Create and manage tender emission scenarios from BOQ quantities"],tender_compare:["Compare Scenarios","Side-by-side comparison of tender emission projections"]};
  const[t,d]=titles[page]||["",""];
  $('pageTitle').textContent=t; $('pageDesc').textContent=d;
  const R={dashboard:renderDashboard,entry_a13:renderEntry,boq_upload:renderBOQUpload,entry_a5:renderA5,approvals:renderApprovals,monthly:renderMonthly,cumulative:renderCumulative,calculations:renderCalculations,baselines:renderBaselines,team:renderTeam,organizations:renderOrganizations,certifications:renderCerts,integrations:renderIntegrations,tender_entry:renderTenderEntry,tender_compare:renderTenderCompare};
